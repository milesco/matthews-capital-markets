          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {ASSET_CLASSES.map((ac) => (
              <Link
                key={ac.label}
                href={ac.href}
                className="group relative rounded-[14px] overflow-hidden flex items-end"
                style={{ height: "220px", background: "#0e1626" }}
              >
                {/* Photo background */}
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                ...
                />
                <span className="relative px-5 pb-5 text-white font-bold text-[15px] leading-tight">
                  {ac.label}
                </span>
              </Link>
            ))}

...
**6,346 URLs** in main sitemap. Breakdown: 3,000 `/properties/{id}/{slug}` listings, 1,502 `/advisors/{slug}`, 406 `/research/{type}/{slug}`, 115 `/about-us/`, 25 `/services/property-types/{type}` (incl. `…/hospitality`), and ~120 named-team microsites (`/the-ruzicka-hotel-team`, `/swon-hotel-group`, `/altman-hotel-group`, `/duong-hospitality-group`, etc.) — each with 10-18 sub-pages. **Every major US MSA gets `/research/market-report/{city}/{city}-2026-investment-forecast-hospitality-market-report`** — Atlanta, Austin, Boston, Charlotte, Chicago, Dallas-FW, Denver, Houston, Las Vegas, etc. Per-property template = 1,453 words, 64 internal links, exclusive-broker section cross-links to advisor pages (excellent pattern). **Zero JSON-LD on sample pages** — glaring miss; the URL slug is what ranks. Per-market report (Austin 1Q 2026) is thin at 454 words — slug carries it. Link-magnets: National Hospitality Group reports, 2026 US Hospitality Investment Forecast.
